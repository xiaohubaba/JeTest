...
Page({
      ...
      success(res) {
        ...
        for (let i = 0; i < res.services.length; i++) {
          if (res.services[i].isPrimary) {
            that.getBLEDeviceCharacteristics(deviceId, res.services[res.services.length - 1].uuid)
            return
          }
        }
      }
  ...
  getBLEDeviceCharacteristics(deviceId, serviceId) {
      ...
      success: (res) => {
        ...
        for (let i = 0; i < res.characteristics.length; i++) {
          ...
          if (item.properties.write) { //向低功耗蓝牙设备特征值中写入二进制数据。注意：必须设备的特征值支持 write 才可以成功调用
            ...
            that.data.characteristicId = item.uuid;
    ...
    wx.onBLECharacteristicValueChange((characteristic) => {
      const idx = that.inArray(that.data.chs, 'uuid', characteristic.characteristicId)
      console.log(characteristic,'读取的值')
      const data = {}
      if (idx === -1) {
        that.data.chs.push({
          uuid: characteristic.characteristicId,
          value: that.ab2hex(characteristic.value)
        });
      } else {
        that.data.chs[idx] = {
          uuid: characteristic.characteristicId,
